...
# 中文评分 ： aistream.cnscore #
  ...
  1. 服务url： rtmp://sandbox.api.aispeech.com/aistream.cnscore.v1.0
  1. 调用参数：
    * scoreType, 2 | 4 | 100 , //评分分制选择。可以是2，4，100中的一个。
    * coreType, cn.word.score.v2.1  表示使用哪个语音内核计算。
    * refText, 参考文本，中文为数字拼音表示， 你好表示为 ni2-hao3。 详细参考[RefText](RefText.md)。
...
```
```
  1. 错误反馈, 如果计算过程出错，getScores得到如下反馈：
    * errID, 错误代码。参考[ErrorCode](ErrorCode.md)。
```
 {
...
```
  1. 对于中文字词类服务，AISpeech API JavaScript SDK 提供了绘制拼音声调曲线的功能。 绘制曲线用到了flot。
...
# 英文单词评分： aistream.enscore #
  1. 服务URL： rtmp://sandbox.api.aispeech.com/aistream.enscore.v1.0
  1. 参数
